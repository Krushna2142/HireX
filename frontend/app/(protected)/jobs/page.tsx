...
function readSavedJobs(): SavedJobPayload[] {
  ...
  try {
    ...
    return parsed
      ...
      .map((item) => item as SavedJobPayload);
  } catch {
    return [];
  }
}
...
function JobCard({
  ...
  onApply: (j: UnifiedJob) => void;
  saved: boolean;
  onToggleSave: (j: UnifiedJob) => void;
}) {
    ...
    >
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 'auto', paddingTop: 4 }}>
        ...
        <button
          ...
          style={{
            fontSize: 13,
            borderColor: saved ? 'rgba(52,211,153,0.35)' : undefined,
            color: saved ? '#34D399' : undefined,
            background: saved ? 'rgba(52,211,153,0.08)' : undefined,
          }}
        >
          {saved ? 'Saved ✓' : 'Save'}
        </button>
    ...
    try {
      ...
      const msg =
        (e as { response?: { data?: { message?: string } } })?.response?.data
          ?.message ?? (e instanceof Error ? e.message : 'Application failed.');
                        ...
                        >
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            >
                              ...
                              {resume.isDefault && (
                                <span style={{ marginLeft: 6, color: '#34D399' }}>
                                  · default
                                </span>
                              )}
                            </p>
...
export default function JobsPage() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      window.removeEventListener('jc:saved-jobs-changed', syncSavedJobs);
    };
  }, []);
  ...
  const getApp = (jobId: string) =>
    applications.find((application) => getApplicationJobId(application) === jobId);
        ...
        {!loading && (
          <p style={{ fontSize: 13, color: 'var(--text-muted)', margin: '4px 0 0' }}>
            ...
            <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.2)' }}>
              {totalLive} live · {PAGE_SIZE} per page · refreshes every 30s
            </span>
          </p>
        )}
      </div>
          ...
          {(
            ...
            <button
              ...
              style={{
                ...
                fontWeight: 500,
                background:
                  sourceFilter === key ? 'rgba(124,58,237,0.2)' : 'transparent',
                color: sourceFilter === key ? '#A78BFA' : 'var(--text-muted)',
                border: 'none',
                borderRight:
                  i < arr.length - 1 ? '1px solid rgba(255,255,255,0.09)' : 'none',
                cursor: 'pointer',
              }}
            >
              {label}
