...
// ✅ camelCase — matches Prisma output from resumes.service.ts listByUser()
interface Resume {
  id:        string;
  fileName?: string;
  createdAt: string;
...
function JobCard({ job, application, onApply }: {
  job:         UnifiedJob;
      ...
      {job.description && (
        <div>
          ...
          {job.description.length > 120 && (
            <button onClick={() => setExpanded(p => !p)} style={{ fontSize: 12, color: '#A78BFA', background: 'none', border: 'none', cursor: 'pointer', padding: '4px 0 0' }}>
...
function ApplyModal({ job, onClose, onSuccess }: {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}
        ...
        {success ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div>
              ...
              {fetching ? (
                ...
                <div style={{ padding: '10px 14px', borderRadius: 8, background: 'rgba(248,113,113,0.08)', border: '1px solid rgba(248,113,113,0.2)', fontSize: 13, color: '#F87171' }}>
                  No resumes found. <a href="/resumes" style={{ color: '#F87171', textDecoration: 'underline' }}>Upload one first →</a>
                </div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {resumes.map(r => (
                    <label key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', borderRadius: 8, border: `1px solid ${resumeId === r.id ? 'rgba(167,139,250,0.5)' : 'rgba(255,255,255,0.08)'}`, background: resumeId === r.id ? 'rgba(167,139,250,0.08)' : 'rgba(255,255,255,0.02)', cursor: 'pointer' }}>
                      ...
                      <div>
                        {/* ✅ camelCase — fileName not file_name */}
            ...
            <div>
              ...
              <textarea value={cover} onChange={e => setCover(e.target.value)} rows={4} placeholder="Why are you a great fit for this role?"
            ...
            {error && (
              <p style={{ margin: 0, fontSize: 13, color: '#F87171', background: 'rgba(248,113,113,0.08)', border: '1px solid rgba(248,113,113,0.2)', borderRadius: 8, padding: '8px 12px' }}>{error}</p>
            )}

            <button onClick={() => void submit()} disabled={loading || !resumeId || resumes.length === 0}
              className="btn w-full" style={{ opacity: (loading || !resumeId || resumes.length === 0) ? 0.5 : 1, fontSize: 14 }}>
...
export default function JobsPage() {
  ...
  useEffect(() => {
    // ✅ no /api/ prefix
    api.get<Application[]>('/jobs/applications/mine')
  ...
  return (
    <section style={{ padding: '2rem 2rem 4rem', maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: '1.5rem', alignItems: 'center' }}>
        ...
        <button onClick={() => void fetchJobs()} disabled={loading}
          style={{ padding: '7px 12px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.09)', borderRadius: 8, fontSize: 12, color: 'var(--text-muted)', cursor: loading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 5 }}>
      ...
      {applications.length > 0 && (
        <div style={{ display: 'flex', gap: 8, marginBottom: '1.25rem', flexWrap: 'wrap' }}>
          ...
          {Object.entries(applications.reduce<Record<string, number>>((acc, a) => {
      ...
      {error ? (
        ...
        <div className="card p-10" style={{ textAlign: 'center' }}>
          <p style={{ fontSize: 14, color: 'var(--text-muted)', margin: 0 }}>No jobs found — try adjusting your filters or refreshing</p>
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 20 }}>
      ...
      {applyTarget && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 50 }}>
          <ApplyModal job={applyTarget} onClose={() => setApplyTarget(null)} onSuccess={handleApplySuccess} />
        </div>
      )}
    </section>
  );
